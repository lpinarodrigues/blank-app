...
def show():
    ...
    if st.session_state.ultima_resposta:
        ...
        if c2.button("🎴 Flashcards"): 
            gerar_apenas_flashcards(d['a'], d['area'], d['sub'], email)
            st.toast("✅ Cards Gerados!")
        if c3.button("📝 Questões"): 
            gerar_apenas_questoes(d['a'], d['area'], d['sub'], email)
            st.toast("✅ Simulado Criado!")
        try:
