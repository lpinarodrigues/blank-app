...
def show():
    ...
    st.markdown("""
        <style>
        .core-header { background: linear-gradient(90deg, #1e293b, #0f172a); padding: 20px; border-radius: 16px; border-bottom: 3px solid #3b82f6; margin-bottom: 25px; text-align: center; color: white; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); }
        .upload-box { background: rgba(30, 41, 59, 0.7); backdrop-filter: blur(10px); border: 1px dashed #475569; border-radius: 12px; padding: 10px; }
        .auto-action-panel { background: rgba(16, 185, 129, 0.1); border: 1px solid #10b981; border-radius: 12px; padding: 20px; margin-top: 15px; margin-bottom: 25px; }
        </style>
    ...
    if doc_upload:
        ...
        if b1.button("📑 Gerar Resumo Estruturado", use_container_width=True, type="primary"):
            st.session_state.auto_prompt = (
                f"Respire fundo. Não tenha pressa. Faça uma análise PROFUNDA E LENTA do documento em anexo. "
                f"PENSE PASSO A PASSO:\n"
                f"Passo 1: Qual é a patologia ou tema central?\n"
                f"Passo 2: Ignore e limpe palavras que o PDF tenha colado ou formatado mal.\n"
                f"Passo 3: Escreva um Protocolo de Excelência dividido rigidamente em: FISIOPATOLOGIA, DIAGNÓSTICO e TRATAMENTO/CONDUTA.\n"
                f"Seja denso no conteúdo, mas organizado na leitura."
            )
            st.rerun()
            
        if b2.button(f"⚡ Extrair {qtd_cards_doc} Cards", use_container_width=True):
            with st.spinner("A analisar o texto profundamente..."):
                texto_puro = ler_documento_universal(doc_upload)[:20000]
                gerados = gerar_apenas_flashcards(texto_puro, "Clínica", "Doc_Automático", email, qtd_cards_doc)
                if gerados > 0: st.success(f"✅ {gerados} Cards Gerados!")
                else: st.error("Erro na extração.")
                
        if b3.button(f"🎯 Criar {qtd_questoes_doc} Questões", use_container_width=True):
            with st.spinner("A formular questões avançadas..."):
                texto_puro = ler_documento_universal(doc_upload)[:20000]
                geradas = gerar_apenas_questoes(texto_puro, "Clínica", "Doc_Automático", email, qtd_questoes_doc)
                if geradas > 0: st.success(f"✅ {geradas} Questões Criadas!")
                else: st.error("Erro na criação.")
    ...
    if prompt_usuario:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Processamento Analítico em andamento..."):
                ...
                if texto_contexto: texto_contexto = texto_contexto[:20000]
    ...
    if st.session_state.ultima_resposta:
        ...
        c_qtd1, c_qtd2 = st.columns(2)
        qtd_cards_chat = c_qtd1.slider("🎴 Flashcards", 1, 20, 5, key="sld_chat_card")
        qtd_questoes_chat = c_qtd2.slider("📝 Questões", 1, 10, 3, key="sld_chat_qst")
        ...
        if c2.button(f"⚡ {qtd_cards_chat} Cards", use_container_width=True): 
            with st.spinner("A processar..."):
                gerar_apenas_flashcards(d['a'], d['area'], d['sub'], email, qtd_cards_chat)
                st.toast(f"✅ Gerados!")
                
        if c3.button(f"🎯 {qtd_questoes_chat} Questões", use_container_width=True): 
            with st.spinner("A processar..."):
                gerar_apenas_questoes(d['a'], d['area'], d['sub'], email, qtd_questoes_chat)
                st.toast(f"✅ Criadas!")
                
        try:
            pdf_bytes = gerar_pdf_premium(d['a'], titulo="Protocolo CORE NEXUS")
            c4.download_button("📄 PDF Seguro", data=pdf_bytes, file_name="Nexus_Protocolo.pdf", mime="application/pdf", use_container_width=True)
        except Exception as e:
            c4.button("📄 Erro PDF", disabled=True, help=str(e))
